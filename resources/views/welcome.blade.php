...
<div class="relative sm:flex sm:justify-center sm:items-center pt-5 bg-center">
    @if (Route::has('login'))
        <div class="sm:fixed sm:top-0 sm:right-0 p-6 text-right z-10 bg-gray-50 w-full border-2 border-bottom">
            <a href="{{ url('/dashboard') }}"
               class="font-semibold text-gray-600 hover:text-gray-900 dark:text-black-600 dark:hover:text-green-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500 pr-5">Dashboard</a>
            <a href="{{ url('/about-us') }}"
               class="font-semibold text-gray-600 hover:text-gray-900 dark:text-black-600 dark:hover:text-green-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500 pr-5">About
                Us</a>

            <a href="{{ url('/faq') }}"
               class="font-semibold text-gray-600 hover:text-gray-900 dark:text-black-600 dark:hover:text-green-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500 pr-5">FAQ</a>

            <a href="{{ route('login') }}"
               class="font-semibold text-gray-600 hover:text-gray-900 dark:text-black-600 dark:hover:text-green-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Log
                in</a>

            @if (Route::has('register'))
                <a href="{{ route('register') }}"
                   class="ml-4 font-semibold text-gray-600 hover:text-gray-900 dark:text-black-600 dark:hover:text-green-600 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Register</a>
            @endif
        </div>
    @endif

    <div class="max-w-7xl p-6 lg:p-8 mt-8">
